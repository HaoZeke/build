...
function settitle()
{
    if [ "$STAY_OFF_MY_LAWN" = "" ]; then
        ...
        if [ -z "$apps" ]; then
            export PROMPT_COMMAND="print -Pn \"\e]0;[${arch}-${product}-${variant}] ${USER}@${HOSTNAME}: ${PWD}\007\""
        else
            export PROMPT_COMMAND="print -Pn \"\e]0;[$arch $apps $variant] ${USER}@${HOSTNAME}: ${PWD}\007\""
        fi
    fi
}
